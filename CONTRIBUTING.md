...
|--------------|------------------------------------------|-----------------------------------|
| `feature/`   | Nouvelle fonctionnalité ou tâche          | `feature/scan-hors-ligne`         |
| `fix/`       | Correction de bug (hors urgence prod)     | `fix/centrage-carte`              |
| `hotfix/`    | Correctif urgent, part de `main`          | `hotfix/crash-au-lancement`       |
| `chore/`     | Maintenance, outillage, dépendances       | `chore/mise-a-jour-rn`            |
...
Exemple : `fix(scan): déduplique les scans QR trop rapprochés`
...
- `Qualité et tests` (lint + tests unitaires Jest)
...
Sont volontairement absents de cette CI (voir `README.md`) : la suite
`parcours` (nécessite une API joignable) et le build Android (long,
dépendant de secrets de signature).
